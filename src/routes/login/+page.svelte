...
<div class="mx-auto flex min-h-[calc(100dvh-3.5rem)] max-w-md flex-col justify-center px-4 py-12">
	<header class="mb-8">
		<h1 class="text-2xl font-semibold text-fg-app">Sign in</h1>
		<p class="mt-2 text-sm text-secondary-app">
			Authenticate with your operator-directory username. Your session binds to the email on
			your account — you cannot change it from this page.
		</p>
	</header>
	...
	<div class="space-y-4">
		...
		<form method="POST" class="space-y-4 rounded-md border border-strong-app bg-surface p-6">
			<label class="block">
				<span class="mb-1.5 block text-sm text-fg-app">Username or email</span>
				<div class="relative">
					<User
